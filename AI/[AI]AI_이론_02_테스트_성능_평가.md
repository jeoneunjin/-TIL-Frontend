...
- 데이터를 **K개 부분집합(fold)**으로 나눈 뒤,(분할 전, **셔플링**먼저! → 중요!)
...
> **레이블(정답) 없이** 데이터의 구조·패턴·집단(잠재 서브그룹)을 찾아내는 학습 <> 지도학습은 정답(레이블)이 있음
...
> 데이터가 고차원이거나 노이즈가 많고, 클러스터링·시각화·전처리를 위해 차원을 줄이고 싶을 때 PCA를 사용
> 데이터가 고차원일 때는 눈으로 확인하기 어려움 따라서 PCA를 통해 차원 축소하여 시각화함
