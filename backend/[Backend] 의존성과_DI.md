...
```
> 만약 `Washer sWasher = ctx.getBean(Washer.class);`로 Washer 타입의 빈을 가져오려면 어떻게 될까? 
> **에러 발생!** 어떤 빈을 줘야하는지(여기에서는 SWasher와 LWasher) 알 수 없기 때문에 에러가 발생함
> 따라서 동일 타입이 여럿일 경우 타입 + 이름 기반으로 빈 조회가 가능함
